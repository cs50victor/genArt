...
sys.path.append('C:/
Users/Admin/Desktop/genArt/utils')
...
studioName, studioSize, 
setStudioX, setStudioY,setStudioZ = "Studio", 64, 2, 2,2


setName, setSiz
e, setScaleX, setScaleY,setScaleZ = "Set", 32, 2, 2, 1


planeName, planeMatName, planeSize = "AbstractPla
ne", "PlaneMat", 8


pSubsurfName, pMarbleTextureName, pSolidifyMod, 
pSmoothMod = "PlaneSubsurf", 'PlaneMarble', "PlaneThickness", "PlaneSmoothness"


bezierCurveName, displaceAxesNam
e, displaceModName = "Bezier", "DisplacementAxes", "PlaneDisplaceMod"


displacePathName = "DisplacementP
ath"


cameraName, backlitName = "MainCamera", "B
acklit"
...
sh(studio.data)
    ...
    blendOperators.object.tran
sform_apply(location=False,
    ...
    y = -setSize/3
    # Create and name atstract plane
ction='SELECT')
    ...
    blendOperators.mesh.primitive_plane_add(size=planeSize,
                                            ...
                                            location=(0, y, z),
                                            scale=(1, 1, 1))
    plane = blendContext.rude_r
egion_shrink_fatten(TRANSFORM_OT_shrink_fatten={
...
oThickness,
    ...
    blendOperators.transform.resize(value=(setS
tudioX, setStudioY, setStudioZ))
...
, 0, studioThickness),
    ...
    mesh.select_all(action='
DESELECT')


    bm = bmesh.from_edit_me
...
JECT')
    ...
    blendOperators.object.transform_appl
    nodes = mat.node_tree.nodes
    ...
    mesh.extrude_region_shrink_fatten(TRANSFORM_OT_shri
nk_fatten={
...
ade_smooth()
    ...
    blendOperators.transform.resize(
value=(setScaleX, setScaleY, setScaleZ))
...
s + setSize/5
    ...
    ensureMode('OBJECT')
    blendOperators.mesh.prim
itive_plane_add(size=planeSize,
...
eMode('EDIT')
    ...
    # blendOperators.mesh.subdivide(number_c
uts= )


    blendOperators.mesh.subdivide(number_
cuts=100)


    # offset=float(-30,30), uniform=float


    blendOpe
rators.transform.vertex_random(
...
plane.location)
    ...
    subSurf.sh
ow_only_control_edges = False


    subSurf.boundary_smooth 
= 'ALL'


    subSurf.uv_smooth = 'SM
OOTH_ALL'
    ...
    # Add marble texture to
 plane


    displace = plane.modifiers.new(name=d
isplaceModName, type='DISPLACE')
...
displaceTexture = blendData.textures.new(pMarbleTextureName, type='MARBLE')
    ...
    displaceTexture.turbulence = floa
t(4.2)


    displaceTexture.noise_depth 
= int(2)
...
ensureMode('OBJECT')
    ...
    # Add smoothness and thickness to p
    cam.rotation_euler = cam.rotation_euler

    solidify = plane.modifiers.new(name=pSolidifyMod, type='
SOLIDIFY')
...
ty_normals = True
    ...
    smoothness = plane.
modifiers.new(name=pSmoothMod, type='SMOOTH')
...
tmode=False,
    ...
    # Create and name alstract plane
    ...
    blendOperators.
                ...
                bezierCurve.data.name = f"{bezierCurveName}_
def ():
    cleanSlate()
    cyclesSettin(s(square'ender=squareIm)
    #  tudio and 
    add()
...
shader.inputs["Base Color"].def
    action()

time_start = time()

    sha
der.inputs["Metallic"].default_value = 1
